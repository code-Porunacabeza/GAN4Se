...
class conv_block(nn.Module):
    def __init__(self, in_ch, out_ch, size,stride,padding):
        super(conv_block, self).__init__()
        self.conv = nn.Conv3d(in_ch, out_ch, size,stride, padding)
        self.gn = nn.GroupNorm(out_ch // 4, out_ch)
        self.relu = nn.ReLU(inplace=True)
...
class NetD(nn.Module):
    def __init__(self):
        ...
        self.convblock1=conv_block(channel_dim,ndf,7,2,3)
        self.convblock2=conv_block(ndf*1,ndf*2,5,2,2)
        self.convblock3=conv_block(ndf*2,ndf*4,4,2,1)
        self.convblock4=conv_block(ndf*4,ndf*8,4,2,1)
        self.convblock5=conv_block(ndf*8,ndf*8,4,2,1)
